...
\resumeSubheading
      ...
      \resumeItemListStart
        \small
\resumeItem{Implemented a dGPU Bootloader boot flow that supports new features with minimal maintenance and overhead in collaboration with PSP internal and external team.}
\resumeItem{Led migration and developed firmware to re-architect BootROM, Bootloader, and Trusted OS to support RISC-V ISA, in addition to ARM A5 ISA.}
...
\resumeItem{Presented a system-level proposal for integrating features such as Static/Dynamic RoT measurements (DRTM) into the TEE OS.}
\resumeItem{Formulated a comprehensive system-level proposal and integrated Device Identifier Composition Engine (DICE) layering architecture for SPDM applications. This initiative covers BootROM, Bootloader and Trusted OS implementations.}
\resumeItem{Design and implemented pre-silicon and post-silicon end-to-end test plan for DICE. Including the the mission mode and provisioning FW.}
\resumeItem{Integrated AMD Crypto engine based on TEE APIs for EC-DSA, EC-DH, RSA-PSS, AES GCM, etc operations for SPDM Open Source project. Designed and implemented creative test units with and without Hyperprovisor donated memory.}
\resumeItem{Customized an AMD X.509 parser for SPDM requirements. Hands on RFC specifications for X.509 standard.}
\resumeItem{Represented the PSP team during the pre-silicon phase of successful programs such as Vega20, Mi100, Mi200 and Mi300 and contributed to their bring-up.}
\resumeItem{Transferred dGPU exclusive features from amd-tee2.0 (AMD proprietary TEE OS) to amd-tee3.0 projects and verified them in the simulator/emulator.}
...
\resumeItem{Collaborated on the design and implementation of a multi-die control network and memory sharing AMD protocol related to PSP.}
\resumeItem{Proposed and implemented a mechanism to minimize boot-up time by utilizing the DMA engine in multi-queue mode.}
...
\resumeItem{Created Python scripts to increase the team throughput in using GDB to simplify debugging of RISC-V/ARM MMU blocks.}
\resumeItem{Created Python scripts to generate certificate signing request (CSR) from DICE parameters and send it to the AMD Key Signing Distribution (KSD) server in a secure manner.}
...
\resumeItem{Mentored numerous newly hired engineers and Co-ops. Acted as one of the first points of contact for the security FW team to provide AMD internal consultations.}
    \resumeItemListEnd
...
\resumeSubheading
      ...
      \resumeItemListStart
        \small
\resumeItem{Contributed as part of a multi-disciplinary team in the design of a Bluetooth Low-Energy IC for IoT and hearing aid applications}
...
\resumeItem{Co-designed and implemented the audio path for a chip with MFi communication capability, including left/right audio synchronization for hearing aids and techniques to reduce artifact effects}
...
\resumeSubheading
        ...
        {REF Electronics}{Tehran, Iran}
      \resumeItemListStart
        \small
\resumeItem{Designed and implemented the electronic components for an infrared camera array with advanced digital image processing capabilities.}
\resumeItem{Created schematics and PCB layouts for analog (buffers, low-noise amplifiers), digital (synchronization), and mixed signals (ADC, DAC) used to drive the infrared detector.}
\resumeItem{Developed innovative non-uniformity correction algorithms for the array sensor on an FPGA, tested through MATLAB simulations.}
\resumeItem{Built a real-time panoramic system capable of scanning 360º using an infrared array, motor, and controlled image packet transmission to a PCI port. Implemented synchronization of the imaging data to the correct position on the FPGA.}
\resumeItem{Redesigned and improved a communication card utilizing a TI DSP for PCM data transmission and receipt through McASP port, and managing call control commands.}
    \resumeItemListEnd
...
\resumeSubheading
      ...
      \resumeItemListStart
        \small
\resumeItem{Created a Media Processor board using PNX1302 by designing and fabricating its schematic and PCB.}
\resumeItem{Designed the schematic and PCB for video components and developed boot loader procedure and device drivers layer.}
\resumeItem{Developed innovative software for image stabilization based on multi local motion estimation for a video processor.}
\resumeItem{Completed application software for MPEG2/4 and JPEG decoding on PNX1005.}
\resumeItem{Designed hardware based on TMS320F28335 and CPLD and implemented device drivers layer for TMS320/Microchip/Atmel microprocessors to read rate sensors. Proposed a high-performance decision mechanism based on DSP concepts.}
\resumeItem{Successfully displayed a gyro-stabilized image with video processing functionalities such as tracking, software stabilization, and OSD after driving a CCD block camera.}
    \resumeItemListEnd
\resumeSubHeadingListEnd
...
\resumeSubHeadingListStart
    ...
    {AMD}{2019 (1 time), 2020 (1 time), 2021 (3 times), 2023 (1 time)}
    \resumeSubheading
...
\begin{itemize}[leftmargin=0.15in, label={}]
    \normalsize{\item{
     \textbf{Programming Languages}{: C (advance), ARM/ RISC-V Assembly (intermediate), Python (intermediate), MATLAB (intermediate), VHDL (beginner), Rust (very beginner)} \\
